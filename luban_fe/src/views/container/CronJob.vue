    ...
    <a-spin :spinning="data.loading" size="large">
      ...
      >
        ...
        <template #action="{text}">
          ...
          <a-dropdown v-if="canAction(text, 'cronjob', 'yaml_edit') || canAction(text, 'cronjob', 'delete')" :trigger="['click']">
            ...
            <template #overlay>
              <a-menu>
                <a-menu-item v-if="canAction(text, 'cronjob', 'yaml_edit')"><span @click="editCronJob(text)">编辑</span></a-menu-item>
                <a-menu-item v-if="canAction(text, 'cronjob', 'yaml_edit')"><span @click="stopCronJob(text)">停止</span></a-menu-item>
                <a-menu-item v-if="canAction(text, 'cronjob', 'delete')"><span @click="removeOneCronJob(text)" style="color: red">删除</span></a-menu-item>
              </a-menu>
            </template>
          </a-dropdown>
      ...
      <div style="float:left;padding: 10px 0 0 20px">
        <a-space>
          <a-button :disabled="!canDeleteSelected" @click="CollectionRemoveCronJob">批量删除</a-button>
        </a-space>
      </div>
    </a-spin>
...
export default {
  ...
  setup(){
    ...
    const canDeleteSelected = computed(() => hasSelected.value && canK8sRowsAction(serviceTreeContext, data.selectedRows, "cronjob", "delete"));
